...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] text-[#F8FAFC] flex flex-col justify-between overflow-x-hidden select-none font-sans scroll-smooth">
      ...
      <footer className="border-t border-[#1E293B] bg-[#0F172A] py-12">
        <div className="max-w-7xl mx-auto px-8 md:px-16 flex flex-col md:flex-row items-center justify-between gap-6 text-xs text-slate-400">
          ...
          <div className="flex space-x-8">
            <button onClick={() => setActiveModal('privacy')} className="hover:text-white transition-colors cursor-pointer focus:outline-none bg-transparent border-none p-0">Privacy Policy</button>
            <button onClick={() => setActiveModal('terms')} className="hover:text-white transition-colors cursor-pointer focus:outline-none bg-transparent border-none p-0">Terms of Service</button>
            <button onClick={() => setActiveModal('support')} className="hover:text-white transition-colors cursor-pointer focus:outline-none bg-transparent border-none p-0">Support</button>
          </div>
      ...
      {/* Dynamic Privacy, Terms & Support Modals */}
      {activeModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-[#131B2E] border border-[#1E293B] text-[#F8FAFC] rounded-xl p-6 max-w-2xl w-full shadow-2xl space-y-4 max-h-[85vh] overflow-y-auto animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-start border-b border-[#1E293B] pb-3">
              <h4 className="text-base font-bold text-white uppercase tracking-wider">
                {activeModal === 'privacy' && 'Privacy Policy'}
                {activeModal === 'terms' && 'Terms of Service'}
                {activeModal === 'support' && 'IntelliSphere Support Desk'}
              </h4>
              <button 
                onClick={() => setActiveModal(null)}
                className="text-slate-400 hover:text-white p-1 hover:bg-slate-800 rounded-lg transition-colors focus:outline-none"
              >
                ✕
              </button>
            </div>
            ...
            <div className="text-xs text-slate-300 space-y-3 leading-relaxed">
              {activeModal === 'privacy' && (
                <>
                  <p className="font-semibold text-white">1. Telemetry and Data Protection</p>
                  <p>At IntelliSphere, we prioritize the confidentiality and safety of your telemetry data. This policy outlines how we ingest, store, and process sensor data, device statuses, and operator action logs.</p>
                  <p className="font-semibold text-white">2. Data Sovereignty</p>
                  <p>All processed sensor records, crop hydration levels, factory OEE reports, and city grids variables remain the exclusive property of your organization. Database tables are strictly isolated under multi-tenant security identifiers.</p>
                  <p className="font-semibold text-white">3. Third-Party Integrations</p>
                  <p>We query the Open-Meteo REST service for real-time meteorological overlays based on cached coordinates. Google Gemini API receives prompts containing parameterized telemetry statistics to output recommendation briefs. No identifying corporate credentials are sent to external services.</p>
                </>
              )}

              {activeModal === 'terms' && (
                <>
                  <p className="font-semibold text-white">1. Acceptable Use of Platform</p>
                  <p>By accessing the IntelliSphere Decision Intelligence workspace, you agree to comply with all operational guidelines and security protocols. Unauthorized attempts to bypass JWT filters are strictly prohibited.</p>
                  <p className="font-semibold text-white">2. AI Telemetry Limitations</p>
                  <p>The predictive summaries, risk forecast indexes, and action checklists generated by the Gemini models are decision-support suggestions only. Operators are responsible for confirming critical physical mitigations (e.g. grid load shunts or hospital triage reallocations).</p>
                  <p className="font-semibold text-white">3. SLA and Service Availability</p>
                  <p>IntelliSphere offers 99.9% uptime targets for enterprise instances. Database queries and PDF OpenPDF generation metrics are backed by automatic Neon database cluster replicas.</p>
                </>
              )}

              {activeModal === 'support' && (
                <>
                  <p className="font-semibold text-white">Need Technical Assistance?</p>
                  <p>Our dedicated platform engineers are ready to help you resolve server connection delays, Gemini API token configurations, or GIS Leaflet map layer queries.</p>
                  <div className="bg-[#0B0F19] border border-[#1E293B] rounded-lg p-4 space-y-2">
                    <p>📨 <strong>Developer Email</strong>: <a href="mailto:support@intellisphere.com" className="text-[#2563EB] hover:underline">support@intellisphere.com</a></p>
                    <p>🕒 <strong>Support Hours</strong>: Monday to Friday, 9:00 AM - 6:00 PM (GMT+5:30)</p>
                    <p>🏢 <strong>Main Office</strong>: IntelliSphere Global Solutions Center, Bangalore, India</p>
                  </div>
                  <p>For urgent production critical system offline escalations, please contact your account manager directly.</p>
                </>
              )}
            </div>
            ...
            <div className="flex justify-end pt-3 border-t border-[#1E293B]">
              <button 
                onClick={() => setActiveModal(null)}
                className="px-4 py-2 bg-[#2563EB] hover:bg-[#2563EB]/95 text-white text-xs font-semibold rounded-lg transition-colors focus:outline-none"
              >
                Understood
              </button>
            </div>
